      ...
      <AnimatePresence mode="wait">
        {pillar ? (
          <motion.div
            ...
            className="rounded-2xl border border-[#c8dae3] bg-white p-10 shadow-[0_8px_30px_-12px_rgba(19,38,58,0.1)]"
          >
            <div className="mb-2 flex items-center gap-2">
              <span className="font-mono text-xs font-semibold tracking-widest text-[#80E4AA]">
                Pillar {pillar.n}
              </span>
            </div>
            <div className="mt-1 mb-3 h-[3px] w-9 rounded-full" style={{ background: pillar.color }} />
            <h3 className="mb-3 text-[1.35rem] font-bold leading-snug text-[#13263A]">
              {pillar.title}
            </h3>
            <p className="text-[1rem] leading-relaxed text-[#4a6070]">{pillar.desc}</p>
          </motion.div>
        ) : (
          <motion.div
            key="empty"
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className="rounded-2xl border border-dashed border-[#c8dae3] p-6 text-center"
          >
...
function Scene({ onPillarSelect }: { onPillarSelect: (i: number) => void }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      scene.add(d1);
      const d2 = new THREE.DirectionalLight(0x80e4aa, 1.0);
      d2.position.set(-3, -2, 2);
      scene.add(d2);
      const d3 = new THREE.DirectionalLight(0x7ec8e3, 0.6);
      d3.position.set(0, -4, -2);
      scene.add(d3);

      // Main icosahedron — glassy light teal, like frosted glass on dark blue
  ...
  return (
    <div className="relative flex-shrink-0" style={{ width: SIZE, height: SIZE }}>
      {/* Dark blue background card behind the shape */}
      <div
      ...
      {PILLARS.map((p, i) => (
        ...
        >
          <span style={{ fontSize: 12, fontWeight: 600, color: p.color, lineHeight: 1 }}>
            {p.title}
          </span>
        </button>
      ))}
      ...
      {/* Hint */}
      <div
        className="pointer-events-none absolute bottom-3 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full px-3 py-1 text-[11px]"
        style={{ background: 'rgba(19,38,58,0.7)', color: '#c8dae3' }}
      >
  ...
  return (
    ...
    >
      <div className="container-custom relative z-10">
        ...
        >
          <p className="mb-3 text-xs font-semibold uppercase tracking-[0.14em] text-[#80E4AA]">
            Benefits
          </p>
          <h2
            id="benefits-heading"
            className="mx-auto mb-4 max-w-2xl text-3xl font-bold leading-tight text-[#13263A] sm:text-4xl"
          >
            Why teams choose{' '}
            <span className="text-[#0a6640]">technology-first growth</span>
          </h2>
          <p className="mx-auto max-w-2xl text-sm leading-relaxed text-[#4a6070] sm:text-base">
