/* ServicesSection.tsx — dark bg matching ProcessSection (#07111a) */
...
const VW = 580;
const VH = 360;
const NODES: [number, number][] = [[80, 300], [290, 180], [520, 60]];
...
function GrowthGraph({ activeIndex }: { activeIndex: number }) {
  ...
  return (
    <svg viewBox={`0 0 ${VW} ${VH}`} width="100%" height="100%" aria-hidden style={{ overflow: 'visible' }}>
      <defs>
        ...
        <clipPath id="area-clip">
          <rect x={0} y={0} width={VW * drawFrac + 2} height={VH + 20}
            style={{ transition: 'width 0.9s cubic-bezier(0.4,0,0.2,1)' }} />
        </clipPath>
      </defs>

      {[0.2, 0.4, 0.6, 0.8].map((t) => (
        <line key={t} x1={28} y1={VH * t} x2={VW - 8} y2={VH * t}
          stroke="rgba(52,211,153,0.08)" strokeWidth={1} />
      ))}
      <line x1={28} y1={4}  x2={28}      y2={VH}stroke="rgba(100,116,139,0.25)" strokeWidth={1} />
      <line x1={28} y1={VH} x2={VW - 8} y2={VH}stroke="rgba(100,116,139,0.25)" strokeWidth={1} />

      {[0.25, 0.5, 0.75].map((t) => (
        <text key={t} x={20} y={VH * t + 4} fontSize={8} textAnchor="end"
      ...
      <polygon
        ...
        fill="url(#g-area)" clipPath="url(#area-clip)"
      />
      <polyline points={PTS} fill="none" stroke="rgba(52,211,153,0.15)" strokeWidth={14}
        strokeLinecap="round" strokeLinejoin="round" filter="url(#g-glow)"
        style={{ strokeDasharray: TOTAL_LEN, strokeDashoffset: dashOffset, transition: LINE_T }} />
      <polyline points={PTS} fill="none" stroke="url(#g-line)" strokeWidth={2.5}
      ...
      {NODES.map(([x, y], i) => {
        ...
        return (
          <g key={i} transform={`translate(${x},${y})`}
            style={{ opacity: reached ? 1 : 0.2, transition: 'opacity 0.5s ease' }}>
            {active && (
              <circle cx={0} cy={0} r={20} fill="rgba(52,211,153,0.10)"
                style={{ animation: 'sv-pulse 2.2s ease-in-out infinite' }} />
            )}
            {reached && (
              <circle cx={0} cy={0} r={active ? 15 : 10} fill="rgba(52,211,153,0.06)"
                filter={active ? 'url(#n-glow)' : undefined}
                style={{ transition: 'r 0.5s ease' }} />
            )}
            <circle cx={0} cy={0}
              r={active ? 9 : past ? 6.5 : 5}
              fill={active ? '#34d399' : past ? '#10b981' : 'rgba(100,116,139,0.25)'}
              stroke={active ? 'rgba(255,255,255,0.5)' : past ? 'rgba(255,255,255,0.3)' : 'none'}
              strokeWidth={active ? 2.5 : 1.5}
              style={{
                transition: 'r 0.45s ease, fill 0.45s ease',
                filter: active
                  ? 'drop-shadow(0 0 10px rgba(52,211,153,0.9))'
                  : past ? 'drop-shadow(0 0 5px rgba(16,185,129,0.5))' : 'none',
              }}
            />
            <text x={0} y={active ? -24 : -18} textAnchor="middle"
              fontSize={active ? 11 : 9} fontWeight={active ? 700 : 500}
              fill={active ? '#34d399' : past ? '#10b981' : 'rgba(100,116,139,0.35)'}
              fontFamily="inherit"
...
// ─── ContentPanel ─────────────────────────────────────────────────────────────
function ContentPanel({ group, index }: { group: ServiceGroup; index: number }) {
  return (
    <AnimatePresence mode="wait">
      ...
      >
        ...
        <h3 className="font-display text-[1.4rem] font-bold leading-tight tracking-tight text-slate-900 mb-5">
          {group.title}
        </h3>
        ...
        <div className="flex flex-col gap-4 flex-1">
          {group.services.map((service) => (
            <div
              ...
              onMouseEnter={(e) => {
                (e.currentTarget as HTMLDivElement).style.border = '1px solid rgba(52,211,153,0.20)';
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="font-semibold text-[0.875rem] text-slate-900 leading-snug">{service.title}</span>
              </div>
              <p className="text-[0.8125rem] leading-[1.6] text-slate-800 mb-3">{service.description}</p>
              <ul className="space-y-1.5" role="list">
                {service.deliverables.map((d) => (
                  <li key={d} className="flex gap-2.5 text-[0.8rem] leading-snug text-slate-900">
                    <span
        ...
        <div className="flex items-center gap-1.5 mt-5">
          {SVC.map((_, i) => (
            <div key={i} className="h-[3px] rounded-full"
              style={{
                flex: i === index ? 4 : 1,
                background: i < index ? '#10b981' : i === index ? '#34d399' : 'rgba(52,211,153,0.15)',
                transition: 'flex 0.5s cubic-bezier(0.4,0,0.2,1), background 0.4s ease',
              }} />
          ))}
        </div>

        <p className="mt-2.5 text-[0.6875rem] text-slate-800 flex items-center gap-1.5">
          {index < COUNT - 1 ? (
            <><span>↓</span> Scroll to advance</>
          ) : (
            <><span style={{ color: '#34d399' }}>✓</span>
              <span style={{ color: '#34d399' }} className="font-medium">Journey complete — scroll to continue</span>
            </>
          )}
        </p>
      </motion.div>
...
export const ServicesSection = () => {
  ...
  const resetTimer = useCallback(() => { startRef.current = Date.now(); setProgress(0); }, []);
  const goTo = useCallback((i: number) => { setActiveIndex(i); resetTimer(); }, [resetTimer]);
...
backdropFilter: 'blur(18px)',
            border: '1px solid rgba(255,255,255,0.07)',
            boxShadow: '0 0 0 1px rgba(52,211,153,0.05), 0 8px 40px -8px rgba(0,0,0,0.7)',
          }}
          onMouseEnter={() => setPaused(true)}
          onMouseLeave={() => setPaused(false)}
        >
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1px 1fr', minHeight: 580 }}>

            {/* LEFT: graph + nav */}
            <div className="flex flex-col p-8" style={{ minHeight: 0 }}>
              <div className="flex flex-col gap-1.5 mb-6">
                {SVC.map((group, i) => (
                  <button key={group.title} onClick={() => goTo(i)}
                    className="flex items-center gap-2.5 text-left w-fit"
                    aria-label={`Go to ${group.title}`}
                    aria-current={i === activeIndex ? 'true' : undefined}>
                    <span className="rounded-full h-[5px] flex-shrink-0" style={{
                      width: i === activeIndex ? 22 : i < activeIndex ? 13 : 7,
                      background: i === activeIndex ? '#34d399' : i < activeIndex ? '#10b981' : 'rgba(52,211,153,0.20)',
                      transition: 'width 0.35s ease, background 0.35s ease',
                    }} />
                    <span style={{
                      fontSize: '0.7rem', lineHeight: 1.35,
                      color: i === activeIndex ? '#34d399' : i < activeIndex ? '#94a3b8' : '#4b5563',
              ...
              <div className="flex justify-between mt-2 px-4">
                <span className="text-[0.6rem] font-medium tracking-wide" style={{ color: 'rgba(52,211,153,0.40)' }}>Foundation</span>
                <span className="text-[0.6rem] font-medium tracking-wide" style={{ color: 'rgba(52,211,153,0.40)' }}>Scalable Business</span>
              </div>
                ...
                <div className="flex items-center gap-1.5 mb-2.5">
                  {SVC.map((_, i) => (
                    <button key={i} onClick={() => goTo(i)}
                      ...
                      style={{ cursor: 'pointer', background: 'rgba(52,211,153,0.12)' }}>
                      <span className="absolute inset-y-0 left-0 rounded-full"
                        style={{
                          width: i < activeIndex ? '100%' : i === activeIndex ? `${progress * 100}%` : '0%',
                          transition: i < activeIndex ? 'width 0.3s ease' : 'none',
                          background: i < activeIndex ? '#10b981' : '#34d399',
                        }} />
                    </button>
                  ))}
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-[0.65rem] text-slate-800">
                    {paused ? '⏸ Paused' : `Auto-advancing · ${activeIndex + 1} of ${COUNT}`}
                  </span>
                  <button onClick={() => setPaused((p) => !p)}
                    className="text-[0.65rem] text-slate-800 hover:text-slate-800 transition-colors">
                    {paused ? '▶ Resume' : '⏸ Pause'}
                  </button>
                </div>
              </div>
            ...
            <div style={{
              ...
              background: 'linear-gradient(to bottom, transparent, rgba(148,163,184,0.35) 15%, rgba(148,163,184,0.35) 85%, transparent)',
...
>
            <span className="max-w-xl text-center text-sm leading-relaxed text-slate-800 sm:text-left">
            ...
            <motion.a href="#contact"
              ...
              onMouseEnter={(e) => { (e.currentTarget as HTMLAnchorElement).style.background = '#2dd4aa'; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLAnchorElement).style.background = '#34d399'; }}
            >
...
background: '#F7FAFC',
            ...
            >
              ...
              <div className="mb-4">
                ...
                <h3 className="font-display text-[1rem] font-bold text-slate-900 leading-snug">{group.title}</h3>
              </div>
              {group.services.map((service) => (
                <div key={service.title} className="mb-4 last:mb-0">
                  <div className="flex gap-3 mb-1.5">
                    ...
                    <span className="font-semibold text-[0.875rem] text-slate-900 pt-1">{service.title}</span>
                  </div>
                  <p className="text-sm leading-relaxed text-slate-800 mb-2">{service.description}</p>
                  <ul className="space-y-1.5">
                    {service.deliverables.map((d) => (
                      <li key={d} className="flex gap-2 text-[0.8125rem] text-slate-900">
                        <Check className="h-3.5 w-3.5 mt-0.5 shrink-0" strokeWidth={2.5} style={{ color: '#34d399' }} />
                        {d}
                      </li>
                    ))}
                  </ul>
        ...
        <div className="mt-8">
          ...
          >
            <span className="text-center text-sm leading-relaxed text-slate-800">{SECTION_SERVICES.cta}</span>
            <motion.a
              href="#contact"
