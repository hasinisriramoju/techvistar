        ...
        return (
          <g key={i} transform={`translate(${x},${y})`}
            ...
            <circle cx={0} cy={0}
              ...
              fill={active ? '#6CD99C' : past ? '#80E4AA' : 'rgba(100,116,139,0.25)'}
            ...
            <text x={0} y={active ? -24 : -18} textAnchor="middle"
              ...
              fill={active ? '#6CD99C' : past ? '#80E4AA' : 'rgba(100,116,139,0.35)'}
              fontFamily="inherit"
      ...
      >
          ...
          {group.services.map((service) => (
            ...
            >
              <div className="flex items-center gap-3 mb-2">
                ...
                <span className="font-semibold text-sm text-[#13263A] leading-snug">{service.title}</span>
              </div>
              <p className="text-base leading-relaxed text-[#475569] mb-3">{service.description}</p>
              <ul className="space-y-1.5" role="list">
                {service.deliverables.map((d) => (
                  <li key={d} className="flex gap-2.5 text-base leading-snug text-[#13263A]">
                    <span
        ...
        <div className="flex items-center gap-1.5 mt-5">
          {SVC.map((_, i) => (
            <div key={i} className="h-[3px] rounded-full"
              style={{
                flex: i === index ? 4 : 1,
                background: i < index ? '#80E4AA' : i === index ? '#6CD99C' : 'rgba(52,211,153,0.15)',
        ...
        <p className="mt-2.5 text-[0.65rem] text-[#475569] flex items-center gap-1.5">
          {index < COUNT - 1 ? (
            ...
            <><span style={{ color: '#6CD99C' }}>✓</span>
              <span style={{ color: '#6CD99C' }} className="font-medium">Journey complete — scroll to continue</span>
            </>
          )}
        </p>
      </motion.div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1px 1fr', minHeight: 580 }}>
            ...
            <div className="flex flex-col p-5 lg:p-8" style={{ minHeight: 0 }}>
                ...
                {SVC.map((group, i) => (
                  <button key={group.title} onClick={() => goTo(i)}
                    ...
                    <span className="rounded-full h-[5px] flex-shrink-0" style={{
                      ...
                      background: i === activeIndex ? '#6CD99C' : i < activeIndex ? '#80E4AA' : 'rgba(52,211,153,0.20)',
                      transition: 'width 0.35s ease, background 0.35s ease',
                    }} />
                    <span style={{
                      fontSize: '0.7rem', lineHeight: 1.35,
                      color: i === activeIndex ? '#6CD99C' : i < activeIndex ? '#94a3b8' : '#4b5563',
              ...
              <div className="flex justify-between mt-2 px-4">
                <span className="text-[0.6rem] font-medium tracking-wide" style={{ color: 'rgba(108,217,156,0.40)' }}>Foundation</span>
                <span className="text-[0.6rem] font-medium tracking-wide" style={{ color: 'rgba(108,217,156,0.40)' }}>Scalable Business</span>
              </div>
...
background: '#F7FAFC',
            ...
            >
              <div
                ...
                style={{ background: 'linear-gradient(to right, #13263A, #6CD99C)' }}
              />
              <div className="mb-4">
                <span
                ...
                <h3 className="font-display text-xl font-bold text-[#13263A] leading-snug">{group.title}</h3>
              </div>
              {group.services.map((service) => (
                <div key={service.title} className="mb-4 last:mb-0">
                  <div className="flex gap-3 mb-1.5">
                    ...
                    <span className="font-semibold text-sm text-[#13263A] pt-1">{service.title}</span>
                  </div>
                  <p className="text-sm leading-relaxed text-[#475569] mb-2">{service.description}</p>
                  <ul className="space-y-1.5">
                    {service.deliverables.map((d) => (
                      <li key={d} className="flex gap-2 text-base text-[#13263A]">
                        <Check className="h-3.5 w-3.5 mt-0.5 shrink-0" strokeWidth={2.5} style={{ color: '#6CD99C' }} />
                        {d}
                      </li>
                    ))}
                  </ul>
        ...
        <div className="mt-8">
          ...
          >
            <span className="text-center text-sm leading-relaxed text-[#475569]">{SECTION_SERVICES.cta}</span>
            <motion.a
              href="#contact"
